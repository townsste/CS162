...
** Description: Validates the user input for the amount of creatures in lineup
** Parameters: None
** Pre-Conditions: None
** Post-Conditions: Input is valid
** Return: Valid input
...
void storyLine(string character)
{
	if (character.compare("1") == 0)  //Vampire Storyline
	{
		...
		cout << "The Suave, Debonair, but Vicious and Surprisingly Resilient!" <<"  VAMPIRE" << endl;
	}
...
int play(Creature * player1, Creature * player2, int &Score1, int &Score2, string team1, string team2)
{
	int attack;
	int defense;
	int damage;
	...
	while (player1->getState() == ALIVE && player2->getState() == ALIVE)
	{
		...
		{
			cout << "\nTeam " << left << setw(12) << team1 << right << setw(4) << "    Team " << left << setw(12) << team2 << endl; //Player output
			...
			cout << right << setw(8) << " " << right << setw(7); //Right justify players defense roll.
...
void closeGameData(Creature *player1, Creature *player2)
{
